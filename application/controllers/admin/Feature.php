...
class Feature extends Render_Controller
{
    public function index()
    {
        // data
        $this->data['judul'] = $this->key_get($this->key_feature_judul);
        $this->data['sub_judul'] = $this->key_get($this->key_feature_sub_judul);
        $this->data['show'] = $this->key_get($this->key_feature_show);
        ...
        $this->breadcrumb_3 = 'Feature';
        $this->breadcrumb_3_url = '#';

        // content
        $this->content      = 'admin/feature';
    ...
    public function update()
    {
        ...
        $judul = $this->key_set($this->key_feature_judul, $main, $show);

        // sub_judul
        $main = $this->input->post('main_sub_judul');
        $show = $this->input->post('show_sub_judul') == null ? 0 : 1;
        $sub_judul = $this->key_set($this->key_feature_sub_judul, $main, $show);

        // perlihatkan konten
        $show_content = $this->key_set($this->key_feature_show, $this->input->post('show_content') == null ? 0 : 1, null);
    ...
    public function insert_foto()
    {
        ...
        $user_id = $this->id;
        $result = $this->model->insert($user_id, $nama, $foto, $status, $keterangan);
    ...
    public function update_foto()
    {
        ...
        $keterangan = $this->input->post("keterangan");
        $user_id = $this->id;
        $result = $this->model->update($id, $user_id, $nama, $foto, $status, $keterangan);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    function __construct()
    {
        ...
        $this->photo_path = './files/image/feature/';
        $this->load->model("admin/FeatureModel", 'model');
