...
class Testimoni extends Render_Controller
{
    ...
    {
        // data
        $this->data['judul'] = $this->key_get($this->key_testimoni_judul);
        $this->data['sub_judul'] = $this->key_get($this->key_testimoni_sub_judul);
        $this->data['show'] = $this->key_get($this->key_testimoni_show);
        ...
        $this->breadcrumb_3 = 'Testimoni';
        $this->breadcrumb_3_url = '#';

        // content
        $this->content      = 'admin/testimoni';
    ...
    public function update()
    {
        ...
        $judul = $this->key_set($this->key_testimoni_judul, $main, $show);

        // sub_judul
        $main = $this->input->post('main_sub_judul');
        $show = $this->input->post('show_sub_judul') == null ? 0 : 1;
        $sub_judul = $this->key_set($this->key_testimoni_sub_judul, $main, $show);

        // perlihatkan konten
        $show_content = $this->key_set($this->key_testimoni_show, $this->input->post('show_content') == null ? 0 : 1, null);
    ...
    public function insert_foto()
    {
        ...
        $user_id = $this->id;
        $result = $this->model->insert($user_id, $nama, $foto, $status, $keterangan, $nilai, $jabatan);
    ...
    public function update_foto()
    {
        ...
        $jabatan = $this->input->post("jabatan");
        $user_id = $this->id;
        $result = $this->model->update($id, $user_id, $nama, $foto, $status, $keterangan, $nilai, $jabatan);
        $code = $result ? 200 : 500;
        $this->output_json(["data" => $result], $code);
    }
    ...
    function __construct()
    {
        ...
        $this->photo_path = './files/image/testimoni/';
